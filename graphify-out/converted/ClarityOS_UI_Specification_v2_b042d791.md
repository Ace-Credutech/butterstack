<!-- converted from ClarityOS_UI_Specification_v2.docx -->
...
Left panel tree architecture  ·  Centre panel tab system  ·  Right panel AI intake agent  ·  Project Confidence Score |
| --- |
...
| --- |
| Module
  └ Submodule         (optional — BA can skip)
      └ Page
          └ Feature      (must have a parent Page) |
| --- |
...
| --- |
| FCS = (D × 0.25) + (E × 0.25) + (A × 0.20) + (S × 0.15) + (V × 0.15)

D — Documentation Score
E — Elicitation Depth Score
A — Assumption Risk Score
S — Stability Score
V — Prototype Validation Score |
| --- |
...
| A = 100 − (sum of risk weights for all unvalidated assumptions)
...
| S = 100 − informal_change_penalties − recency_decay

Recency decay = min(30, days_since_last_validation × 1.5)
Floor: 0 — cannot go negative. |
| --- |
| Signal | Penalty |
| --- | --- |
...
| V = (Prototype_generated × 0.20) + (Client_viewed × 0.25)
  + (Reaction_captured × 0.30) + (Reaction_incorporated × 0.25) |
| --- |
| Stage | Points |
| --- | --- |
...
| Project Score = average of all Feature Confidence Scores across all modules

Module Score = average of all FCS in that module,
               with bottom 20% of features weighted 2× |
| --- |
| Card | What it shows |
| --- | --- |
